...
This guide shows how to run the ROBOTIS Motion Controller from [`robotis_motion_controller`](https://github.com/ROBOTIS-GIT/robotis_motion_controller) on OMX.
...
![omx_motion_controller](/simulation/omx/omx_motion_controller.png)
...
![default_vs_movel_omx](/simulation/omx/default_vs_movel_omx.gif)
...
1. In the first terminal, launch the default ROBOTIS Motion Controller.
   ```bash
   ros2 launch robotis_motion_controller_ros omx_controller.launch.py
   ```
...
```bash
ros2 launch robotis_motion_controller_ros omx_controller.launch.py controller_type:=movej
```
...
![movej_control](/simulation/omx/omx_movej.gif)
...
The main parameters live in `robotis_motion_controller_ros/config/omx_config.yaml`. The file is divided by controller name, so you usually tune only the block that matches the controller mode you are running.
