...
prev: 
  text: Setup Guide
  link: /omy/setup_guide_omy
---

# Motion Controller

This guide shows how to run the motion controller from [`robotis_motion_controller`](https://github.com/ROBOTIS-GIT/robotis_motion_controller/tree/feature-qp?tab=readme-ov-file) on OMY.

The default `omy` controller follows a task-space end-effector goal. In practice, it is mainly used with an interactive marker in RViz. The `movej` and `movel` controllers are command-based modes for joint-space and Cartesian-space motion.

## Supported Controllers

- `controller_type:=omy`: Main task-space controller. It tracks one end-effector pose goal and can be used with an RViz interactive marker.
...
- Make sure `robotis_interfaces` is available in the same ROS 2 workspace.
...
## Launch the Default OMY Controller
...
When `start_interactive_marker:=true`, the launch file starts one interactive marker named `omy_goal_marker` and publishes its pose to `/omy_eef_goal_pose`.

## Operating the OMY Controller from RViz

1. Launch the `omy` controller with `start_interactive_marker:=true`.
...
![marker_control](/simulation/omy/omy_interactive_marker_real.gif)

::: tip
The default `omy` controller captures the current end-effector pose on startup and then follows the marker goal. Small, continuous marker motions usually give the smoothest motion.
:::
...
```bash
ros2 launch motion_controller_ros omy_controller.launch.py controller_type:=movel
```

It subscribes to `~/movel` under the node namespace, which becomes `/omy_movel_controller/movel` with the default node name.
...
![movel_control](/simulation/omy/omy_movel.gif)

## Useful Launch Arguments

- `controller_type`: Selects `omy`, `movej`, or `movel`.
- `start_interactive_marker`: Starts the interactive marker when `controller_type:=omy`.
...
- `marker_goal_topic`: Goal topic published by the interactive marker. Default: `/omy_eef_goal_pose`.
...
| --- | --- | --- |
| `/joint_states` | Topic | Measured OMY joint state required by all controller modes |
| `/omy_eef_goal_pose` | Topic | Interactive-marker goal pose for `controller_type:=omy` |
| `/leader/joint_trajectory` | Topic | Output joint trajectory command published by the controllers |
| `/omy_controller/current_pose` | Topic | Current end-effector pose published by the default controller |
| `/omy_controller/controller_error` | Topic | Controller error status for the default controller |
| `/omy_movej_controller/movej` | Topic | Joint-space command topic for `movej` |
| `/omy_movel_controller/movel` | Topic | Cartesian command topic for `movel` |
...
- If `movej` does not respond, confirm the published joint names match the OMY model joints and that `time_from_start` is greater than zero.
- If `movel` does not respond, confirm the `MoveL` command is being published to `/omy_movel_controller/movel`.
- If the controller reports a QP solver failure, reduce the requested motion range or command a pose closer to the current configuration.
